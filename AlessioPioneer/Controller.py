...
class Controller:
    ...
    def act(self):
        if self.my_state == "Stopped" and int(float((self.front_distance_detected))) > 100:
            Logger.info("Taking action to start motors")
            self.my_state = "Running"
            RH.send_data(R, "commands", "Motor_Status", "Ahead")
        if self.my_state == "Backing" and int(float((self.front_distance_detected))) > 100:
            Logger.info("Taking action to go ahead again")
            self.my_state = "Running"
            RH.send_data(R, "commands", "Motor_Status", "Ahead")
        if self.my_state == "Running" and int(float((self.front_distance_detected))) < 100:
